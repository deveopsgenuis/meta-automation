...
<template>
    <GuestLayout :title="$t('privacy.page_title')">
        <Head :title="$t('privacy.page_title')" />

        <div class="prose prose-neutral dark:prose-invert max-w-none">
            <p class="text-sm text-muted-foreground">{{ $t('privacy.last_updated') }}</p>

            <p>{{ $t('privacy.intro') }}</p>

            <div v-for="(section, key) in $t('privacy.sections')" :key="key" class="mt-8">
